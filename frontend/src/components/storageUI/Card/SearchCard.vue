      ...
      <CardContent class="h-full w-full flex flex-col p-4">
        ...
        <ScrollArea class="flex-1 w-full mt-4">
          ...
          <div v-else class="space-y-2 w-full pr-3">
            <Card v-for="(item, index) in files" :key="item.cid || index"
                ...
                <div class="w-full flex flex-col">
                  ...
                  <div class="flex items-center gap-4 w-full">
                    ...
                    <div class="flex-none flex gap-2">
                      <Button v-if="item.cid" variant="ghost" size="icon"
                        @click.prevent="openLink(getIpfsUrl(item.cid))" title="Open in new tab" class="h-8 w-8">
                        <ExternalLink class="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" @click.prevent="copyFileLink(item)" title="Copy link"
                        class="h-8 w-8">
                  ...
                  <div v-if="item.cid" class="mt-3 w-full">
                    <div class="w-full relative">
                      <Input :model-value="getIpfsUrl(item.cid)" readonly
