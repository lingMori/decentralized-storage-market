...
export function createFileUploadedEvent(
  ...
  fileUploadedEvent.parameters.push(
    new ethereum.EventParam(
      "storageSource",
      ethereum.Value.fromString(storageSource)
    )
  )
  fileUploadedEvent.parameters.push(
...
export function createLoadIncreasedEvent(
  owner: Address,
  additionalLoad: BigInt
): LoadIncreased {
  let loadIncreasedEvent = changetype<LoadIncreased>(newMockEvent())

  loadIncreasedEvent.parameters = new Array()

  loadIncreasedEvent.parameters.push(
    new ethereum.EventParam("owner", ethereum.Value.fromAddress(owner))
  )
  loadIncreasedEvent.parameters.push(
    new ethereum.EventParam(
      "additionalLoad",
      ethereum.Value.fromUnsignedBigInt(additionalLoad)
    )
  )

  return loadIncreasedEvent
}
